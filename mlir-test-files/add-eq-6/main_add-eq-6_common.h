...
int run(FuncGenerator generateCryptoContext,
  ...
  printModulusChain(cc, "add-eq-6", selectionApproach);
  saveParamsToJsonFile(cc, "add-eq-6", selectionApproach);
  ...
  // OpenFHE currently evaluates add-eq-6 to -7740 for this input pattern.
  std::vector<int16_t> expected_vector(8, -7740);

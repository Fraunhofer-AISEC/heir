...
int run(FuncGenerator generateCryptoContext,
  ...
  printModulusChain(cc, "form-equal", selectionApproach);
  saveParamsToJsonFile(cc, "form-equal", selectionApproach);
  ...
  for (int i = 0; i < 64; i++) {
    ...
    std::vector<int16_t> arg(8, i == 32 ? 1 : 0);
    args.push_back(arg);
  }

  // For form-equal with one-hot inputs (arg0=1, others=0), the plaintext
  // reference result for each slot is 677.
  std::vector<int16_t> expected_vector(8, 5);

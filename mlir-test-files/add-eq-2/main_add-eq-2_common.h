...
int run(FuncGenerator generateCryptoContext,
  ...
  printModulusChain(cc, "add-eq-2", selectionApproach);
  saveParamsToJsonFile(cc, "add-eq-2", selectionApproach);
  ...
  // OpenFHE currently evaluates add-eq-2 to 8 for this input pattern.
  std::vector<int16_t> expected_vector(8, 8);

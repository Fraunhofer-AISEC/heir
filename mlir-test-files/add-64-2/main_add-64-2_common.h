#ifndef ADD_64_2_COMMON_H
#define ADD_64_2_COMMON_H
...
int run(FuncGenerator generateCryptoContext,
  ...
  printModulusChain(cc, "add-64-2", selectionApproach);
  saveParamsToJsonFile(cc, "add-64-2", selectionApproach);
  ...
  for (int i = 0; i < 64; i++) {
    ...
    std::vector<int16_t> arg(8, i % 32 == 0 ? 1 : 0);
    args.push_back(arg);
  }

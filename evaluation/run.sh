...
if [ "$use_new_layout" = true ]; then
    bazel run //tools:heir-opt -- "$PWD/$input_file" --pass-pipeline='builtin.module(
        secretize,
        wrap-generic,
        select-rewrite,
    ...
    bazel run //tools:heir-opt -- --secretize --mlir-to-secret-arithmetic --mlir-print-ir-after-all --mlir-print-ir-tree-dir="$target_dir" "$PWD/$input_file" > "$target_dir/${filename}.secretized_arith.mlir"
fi
...
bazel run //tools:heir-opt -- --mlir-to-bgv="ciphertext-degree="1024" noise-model=bgv-noise-mono" --scheme-to-openfhe="entry-function=main" --mlir-print-ir-after-all --mlir-print-ir-tree-dir="$target_dir/mlir-to-bgv" "$target_dir/${filename}.secretized_arith.mlir" > "$target_dir/${filename}.bgv.mlir"
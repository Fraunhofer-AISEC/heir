module {
    func.func @ctr_mode(%key: tensor<16xi8>, %iv: tensor<16xi8>, %data: tensor<50x16xi8>) -> (tensor<50x16xi8>) {
        ...
        %data_len = tensor.dim %data, %c0 : tensor<50x16xi8>

        %round_keys = func.call @key_expansion(%key) : (tensor<16xi8>) -> tensor<11x16xi8>

        %enc_loop_data, %looped_iv = scf.for %i = %c0 to %data_len step %c1 iter_args(%enc_data = %data, %cur_iv = %iv) -> (tensor<50x16xi8>, tensor<16xi8>) {

            %encrypted_block = func.call @encrypt_block(%cur_iv, %round_keys) : (tensor<16xi8>, tensor<11x16xi8>) -> tensor<16xi8>
            %data_block = tensor.extract_slice %data [%i, 0] [1, 16] [1, 1] : tensor<50x16xi8> to tensor<16xi8>

            %data_iter = tensor.insert_slice %encrypted_block into %enc_data [%i, 0] [1, 16] [1, 1] : tensor<16xi8> into tensor<50x16xi8>
            ...
            scf.yield %data_iter, %next_iv : tensor<50x16xi8>, tensor<16xi8>
        } {lower = 0, upper = 64}
        // Return the result
        return %enc_loop_data : tensor<50x16xi8>
    }

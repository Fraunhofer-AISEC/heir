module {
func.func @main(%input: tensor<784xi32> ,
               %layer1_weights: tensor<512x784xi32>,
               %layer1_bias: tensor<512xi32>,
               %layer2_weights: tensor<10x512xi32>,
               %layer2_bias: tensor<10xi32>) -> tensor<512xi32> {
  ...
  %layer1_empty = tensor.empty() : tensor<512xi32>

  %layer1_out = scf.for %i = %c_0_idx to %c_512 step %c_one iter_args(%out = %layer1_empty) -> tensor<512xi32> {
    ...
    %acc_final = scf.for %j = %c_0_idx to %c_784 step %c_one iter_args(%acc = %c_0_i32) -> (i32) {
      %weight = tensor.extract %layer1_weights[%i, %j] : tensor<512x784xi32>
      %input_val = tensor.extract %input[%j] : tensor<784xi32>
    ...
    // Bias hinzufügen
    %bias = tensor.extract %layer1_bias[%i] : tensor<512xi32>
    ...
    // Truncieren auf i8
    %out_next = tensor.insert %relu_value into %out[%i] : tensor<512xi32>
    scf.yield %out_next : tensor<512xi32>
  }

  func.return %layer1_out : tensor<512xi32>
}
}